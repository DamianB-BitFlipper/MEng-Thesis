...
A website, which assumes the more advanced threat model as described, would use transaction authentication to prevent any high-risk operations from being maliciously issued on behalf of the user. Such high-risk operations include, but are not limited to, deleting one's account, transferring money, managin administrative permissions, publishing important software releases, etc. 

% TODO: Give more details in each example. 
% Explain how txAuthn shows a message to the user, and the user authenticates on-device

Although the webauthn describes transaction authentication in the protocol specification, no serious service has rolled out support for such functionality. Apart from not being integrated into real-world applications, there also has been no investigation into the implications of where transaction authentication fits well, where it is inhibitory or unnecessary, what it takes to support transaction authentication in web-service and any software system designs that work well to integrate and configure webauthn for an existing service.
